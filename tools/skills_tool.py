...
# All skills live in <HERMES_HOME>/skills/ (seeded from bundled skills/ on
# install). This is the single source of truth -- agent edits, hub installs,
# and bundled skills all coexist here without polluting the git repo.
#
# HERMES_HOME / SKILLS_DIR are resolved DYNAMICALLY (not cached at import) so a
# per-job profile override (cron jobs with a `profile` field set a context-local
# Hermes home) is honored at call time. Caching them at import froze the long-
# lived gateway/scheduler process to whatever profile was active on first import
# (usually `default`), so a biglobster/finview profile cron could never see its
# own profile-scoped skills. Internal code must call ``_skills_dir()`` /
# ``_hermes_home()`` rather than the bare names; the module ``__getattr__`` below
# exposes ``HERMES_HOME`` / ``SKILLS_DIR`` for external readers and test patching.
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    """
    # Try the active skills dir first (respects monkeypatching in tests),
