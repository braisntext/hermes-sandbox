...
# HERMES_HOME / SKILLS_DIR / MANIFEST_FILE are resolved DYNAMICALLY (not cached
# at import) so a per-job profile override (cron jobs with a `profile` field set
# a context-local Hermes home) seeds/syncs the correct profile. Internal code
# calls the ``_hermes_home()`` / ``_skills_dir()`` / ``_manifest_file()`` helpers;
...
def _manifest_file() -> Path:
    patched = globals().get("MANIFEST_FILE")
    return patched if patched is not None else _skills_dir() / ".bundled_manifest"
...
def __getattr__(name: str):
    ...
    if name == "MANIFEST_FILE":
        return get_hermes_home() / "skills" / ".bundled_manifest"
