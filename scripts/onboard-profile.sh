#!/bin/sh
# onboard-profile.sh — interactive Hermes profile onboarding
#
# Run INSIDE the container as root or hermes:
#   sh /opt/hermes/scripts/onboard-profile.sh
#
# What it does:
#   1. Creates a fresh Hermes profile (hermes profile create)
#   2. Sets up git credentials scoped to the profile home
#   3. Clones the project repo into the profile workspace
#   4. Syncs shared API keys from the main .env
#   5. Adds the Telegram topic → profile routing entry to config.yaml
#   6. Writes gateway_state.json so the profile gateway auto-starts on next boot
#   7. Runs a quick smoke test via /api/delegate

set -e

HERMES_HOME="${HERMES_HOME:-/opt/data}"
INSTALL_DIR="/opt/hermes"
PY="$INSTALL_DIR/.venv/bin/python3"
PROFILES_ROOT="$HERMES_HOME/profiles"

# ── helpers ────────────────────────────────────────────────────────────────

as_hermes() {
    if [ "$(id -u)" = 0 ]; then
        s6-setuidgid hermes "$@"
    else
        "$@"
    fi
}

ask() {
    _prompt="$1"
    _default="$2"
    if [ -n "$_default" ]; then
        printf '%s [%s]: ' "$_prompt" "$_default"
    else
        printf '%s: ' "$_prompt"
    fi
    read -r _val
    printf '%s' "${_val:-$_default}"
}

ask_required() {
    _label="$1"
    _val=""
    while [ -z "$_val" ]; do
        _val=$(ask "$_label" "")
        [ -z "$_val" ] && echo "  ↳ Required — cannot be blank."
    done
    printf '%s' "$_val"
}

die() { echo "ERROR: $*" >&2; exit 1; }

# ── sanity check ───────────────────────────────────────────────────────────

[ -d "$HERMES_HOME" ] || die "HERMES_HOME ($HERMES_HOME) not found. Run this inside the container."
[ -f "$PY" ] || die "Python venv not found at $PY."

# ── banner ─────────────────────────────────────────────────────────────────

echo ""
echo "╔══════════════════════════════════════╗"
echo "║   Hermes Profile Onboarding Script   ║"
echo "╚══════════════════════════════════════╝"
echo ""

# ── 1. gather inputs ───────────────────────────────────────────────────────

echo "── Profile ─────────────────────────────"
PROFILE_NAME=$(ask_required "Profile name (slug, e.g. finview)")

PROFILE_DIR="$PROFILES_ROOT/$PROFILE_NAME"
if [ -d "$PROFILE_DIR" ]; then
    echo "  ⚠  Profile directory already exists: $PROFILE_DIR"
    _overwrite=$(ask "Continue and overwrite?" "n")
    [ "$_overwrite" = "y" ] || [ "$_overwrite" = "Y" ] || { echo "Aborted."; exit 0; }
fi

echo ""
echo "── Git repo ─────────────────────────────"
echo "  Leave blank to skip repo clone."
REPO_URL=$(ask "GitHub repo HTTPS URL" "")

echo ""
echo "── Telegram routing ─────────────────────"
echo "  The default gateway routes messages from a forum topic to this profile."
echo "  Find the thread_id in the Telegram message URL: t.me/c/<chat>/<thread_id>/<msg_id>"
echo ""

# Auto-detect existing chat_id from config.yaml
EXISTING_CHAT_ID=$(as_hermes "$PY" - <<'PYEOF' 2>/dev/null
import os, yaml
try:
    cfg = yaml.safe_load(open(os.environ.get("HERMES_HOME","/opt/data") + "/config.yaml"))
    topics = cfg.get("telegram", {}).get("extra", {}).get("group_topics", [])
    if topics:
        print(str(topics[0].get("chat_id", "")))
except Exception:
    pass
PYEOF
)

CHAT_ID=$(ask "Telegram group chat_id (negative number)" "${EXISTING_CHAT_ID:-}")
THREAD_ID=$(ask_required "Telegram thread_id for the $PROFILE_NAME topic (integer)")
TOPIC_DISPLAY=$(ask "Topic display name" "$PROFILE_NAME")

echo ""
echo "── Summary ──────────────────────────────"
echo "  Profile name : $PROFILE_NAME"
echo "  Profile dir  : $PROFILE_DIR"
echo "  Repo URL     : ${REPO_URL:-(skip)}"
echo "  Chat ID      : ${CHAT_ID:-(skip routing)}"
echo "  Thread ID    : $THREAD_ID"
echo "  Topic name   : $TOPIC_DISPLAY"
echo "─────────────────────────────────────────"
printf 'Proceed? [y/N]: '
read -r _confirm
[ "$_confirm" = "y" ] || [ "$_confirm" = "Y" ] || { echo "Aborted."; exit 0; }
echo ""

# ── 2. create profile ──────────────────────────────────────────────────────

echo "[1/7] Creating profile '$PROFILE_NAME'..."
if as_hermes hermes profile create "$PROFILE_NAME" 2>/dev/null; then
    echo "      ✓ hermes profile create succeeded"
else
    echo "      ↳ CLI failed — creating profile dirs manually"
    as_hermes mkdir -p \
        "$PROFILE_DIR/memories" \
        "$PROFILE_DIR/sessions" \
        "$PROFILE_DIR/skills" \
        "$PROFILE_DIR/workspace" \
        "$PROFILE_DIR/cron" \
        "$PROFILE_DIR/home"
    # Create SOUL.md (required marker for the s6 reconciler and boot script)
    as_hermes sh -c "printf '# %s — Hermes profile\n\nYou are the Hermes agent for the **%s** project.\n' '$PROFILE_NAME' '$PROFILE_NAME' > '$PROFILE_DIR/SOUL.md'"
    echo "      ✓ profile dirs created manually"
fi

# ── 3. git credentials scoped to profile home ─────────────────────────────

echo "[2/7] Setting up git credentials for profile..."
GITHUB_TOKEN="${GITHUB_TOKEN:-$(grep -m1 '^GITHUB_TOKEN=' "$HERMES_HOME/.env" 2>/dev/null | cut -d= -f2-)}"
if [ -n "$GITHUB_TOKEN" ]; then
    HOME="$PROFILE_DIR" as_hermes git config --global credential.helper store
    HOME="$PROFILE_DIR" as_hermes git config --global user.name "Hermes Agent"
    HOME="$PROFILE_DIR" as_hermes git config --global user.email "hermes@agent.local"
    printf 'https://x-access-token:%s@github.com\n' "$GITHUB_TOKEN" | \
        as_hermes sh -c "cat > '$PROFILE_DIR/.git-credentials'"
    chmod 600 "$PROFILE_DIR/.git-credentials"
    echo "      ✓ git credentials written (.git-credentials)"
else
    echo "      ⚠ GITHUB_TOKEN not found — git clone over HTTPS may fail"
fi

# ── 5. clone repo into workspace ───────────────────────────────────────────

if [ -n "$REPO_URL" ]; then
    echo "[3/7] Cloning repo..."
    _repo_name=$(basename "$REPO_URL" .git)
    _workspace="$PROFILE_DIR/workspace"
    as_hermes mkdir -p "$_workspace"
    if [ -d "$_workspace/$_repo_name/.git" ]; then
        echo "      ↳ repo already cloned — pulling latest"
        HOME="$PROFILE_DIR" as_hermes git -C "$_workspace/$_repo_name" pull --ff-only
    else
        HOME="$PROFILE_DIR" as_hermes git clone "$REPO_URL" "$_workspace/$_repo_name"
    fi
    echo "      ✓ repo at $_workspace/$_repo_name"
else
    echo "[3/7] Skipping repo clone (no URL given)"
fi

# ── 6. sync shared API keys into profile .env ─────────────────────────────

echo "[4/7] Syncing API keys into profile .env..."
as_hermes "$PY" - <<PYEOF
import os, re
from pathlib import Path

home = Path("$HERMES_HOME")
profile_env = Path("$PROFILE_DIR") / ".env"

inject = [
    "OPENROUTER_API_KEY", "HERMES_CALLBACK_SECRET", "HERMES_CALLBACK_URL",
    "HERMES_MAX_ITERATIONS", "EXA_API_KEY", "HUGGINGFACE_API_KEY",
    "GITHUB_TOKEN", "AUXILIARY_VISION_MODEL",
]

content = profile_env.read_text(encoding="utf-8") if profile_env.exists() else ""
for var in inject:
    val = os.environ.get(var, "")
    if not val:
        # fall back to reading from main .env
        m = re.search(rf"^{re.escape(var)}=(.*)", open(home / ".env").read(), re.MULTILINE)
        val = m.group(1).strip() if m else ""
    if not val:
        continue
    if re.search(rf"^{re.escape(var)}=", content, re.MULTILINE):
        content = re.sub(rf"^{re.escape(var)}=.*", f"{var}={val}", content, flags=re.MULTILINE)
    else:
        sep = "" if (not content or content.endswith("\n")) else "\n"
        content += f"{sep}{var}={val}\n"

profile_env.write_text(content, encoding="utf-8")
print(f"      ✓ synced keys into {profile_env}")
PYEOF

# ── 7. add Telegram topic routing to default config.yaml ──────────────────

if [ -n "$CHAT_ID" ] && [ -n "$THREAD_ID" ]; then
    echo "[5/6] Adding topic routing to config.yaml..."
    as_hermes "$PY" - <<PYEOF
import yaml
from pathlib import Path

config_path = Path("$HERMES_HOME") / "config.yaml"
cfg = yaml.safe_load(config_path.read_text()) or {}

chat_id = "$CHAT_ID"
try:
    chat_id = int(chat_id)
except ValueError:
    pass

thread_id = int("$THREAD_ID")
profile_name = "$PROFILE_NAME"
topic_name = "$TOPIC_DISPLAY"

new_entry = {"thread_id": thread_id, "name": topic_name, "profile": profile_name}

tg = cfg.setdefault("telegram", {})
extra = tg.setdefault("extra", {})
group_topics = extra.setdefault("group_topics", [])

# Find the chat entry for this chat_id, or create it
chat_entry = next((e for e in group_topics if str(e.get("chat_id", "")) == str(chat_id)), None)
if chat_entry is None:
    chat_entry = {"chat_id": chat_id, "topics": []}
    group_topics.append(chat_entry)

topics = chat_entry.setdefault("topics", [])
existing = next((t for t in topics if t.get("thread_id") == thread_id), None)
if existing:
    existing.update(new_entry)
    print("      ✓ updated existing thread_id entry")
else:
    topics.append(new_entry)
    print(f"      ✓ added thread_id={thread_id} → profile={profile_name}")

config_path.write_text(yaml.dump(cfg, default_flow_style=False, allow_unicode=True))
print(f"      ✓ config.yaml written")

# Verify it landed in the right place (not under display.telegram.extra)
cfg2 = yaml.safe_load(config_path.read_text())
found = cfg2.get("telegram", {}).get("extra", {}).get("group_topics", [])
print(f"      ✓ verified: telegram.extra.group_topics has {len(found)} chat entries")
PYEOF
else
    echo "[5/6] Skipping topic routing (no chat_id or thread_id provided)"
fi

# ── 8. gateway state ───────────────────────────────────────────────────────

echo "[6/6] Setting gateway auto-start state..."
as_hermes sh -c "printf '{\"gateway_state\": \"running\"}\n' > '$PROFILE_DIR/gateway_state.json'"
echo "      ✓ gateway_state.json → running"
echo "      ↳ The s6 reconciler will register gateway-$PROFILE_NAME on next container restart."
echo "      ↳ To register it NOW without restarting, run:"
echo "         /command/s6-svc -u /run/service/gateway-$PROFILE_NAME"
echo "         (only works if cont-init 02-reconcile-profiles ran first)"

# ── smoke test ─────────────────────────────────────────────────────────────

echo ""
echo "── Smoke test ───────────────────────────"
_resp=$(curl -sf --max-time 10 \
    -X POST http://localhost:9119/api/delegate \
    -H "Content-Type: application/json" \
    -d "{\"task\": \"say hello\", \"profile\": \"$PROFILE_NAME\"}" 2>/dev/null || echo "CURL_FAIL")

if echo "$_resp" | grep -q "accepted"; then
    echo "  ✓ /api/delegate profile=$PROFILE_NAME → accepted"
elif [ "$_resp" = "CURL_FAIL" ]; then
    echo "  ⚠ Could not reach dashboard on localhost:9119 — test manually:"
    echo "    curl -X POST http://localhost:9119/api/delegate \\"
    echo "      -H 'Content-Type: application/json' \\"
    echo "      -d '{\"task\": \"say hello\", \"profile\": \"$PROFILE_NAME\"}'"
else
    echo "  ⚠ Unexpected response: $_resp"
fi

# ── done ───────────────────────────────────────────────────────────────────

echo ""
echo "╔══════════════════════════════════════════════════════╗"
echo "║  Done! Profile '$PROFILE_NAME' is ready.              "
echo "║                                                        "
echo "║  Next steps:                                           "
echo "║  1. Restart container to register the s6 gateway.     "
echo "║  2. Send a message to thread $THREAD_ID in Telegram.   "
echo "║  3. Tail gateway logs:                                 "
echo "║     tail -f /opt/data/logs/gateways/default/current   "
echo "╚══════════════════════════════════════════════════════╝"
echo ""
