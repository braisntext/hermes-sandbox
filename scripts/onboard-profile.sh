...
echo ""
echo "── Project ──────────────────────────────────────────────"
echo "  One or two sentences describing what the project does."
PROJECT_DESC=$(ask_required "Project description")
...
# Append project scope + shared work style to SOUL.md.
# Uses >> so the default content generated by hermes profile create is preserved.
_repo_basename=$(basename "${REPO_URL:-.git}" .git)
_workspace_path="$PROFILE_DIR/workspace/$_repo_basename"
...
- Only operate on this project. Do not reference, report on, or act on other profiles or projects (grow-shop, biglobster, etc.).
...
SOULEOF
echo "      ✓ SOUL.md updated with project scope and work style"
