...
# BigLobster fork: per-boot env sync + config key enforcement + memory seed
# + egress check (replaces the pre-s6 entrypoint.sh env-patching block).
COPY --chmod=0755 docker/cont-init.d/03-biglobster-config /etc/cont-init.d/03-biglobster-config
...
# Keep the container alive as the s6 "main program" so the supervised
# dashboard service (HERMES_DASHBOARD=1) and per-profile gateways keep
# serving. main-wrapper.sh sees `sleep` as a bare executable and execs it
# directly; the dashboard + gateways run as independent s6 services
# alongside. Overrides upstream's `CMD [ ]` (which would run bare `hermes`
# and exit in a non-TTY container, terminating the container).
CMD ["sleep", "infinity"]
